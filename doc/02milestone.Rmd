...
g <- ggplot(data = data.frame(x = 1:dsize, y = freqs[1:dsize] / sum(freqs), z = as.factor(rep(1, dsize))), 
    ...
    geom_text(mapping = aes(x = sufficient50_1, y = 5e-7, label = "50% 1-grams"), angle = 90, size = 4, vjust = -0.4) +
    geom_text(mapping = aes(x = sufficient90_1, y = 5e-7, label = "90% 1-grams"), angle = 90, size = 4, vjust = -0.4) +
...
g <- g + geom_line(data = data.frame(x = 1:dsize, y = freqs[1:dsize] / sum(freqs), z = as.factor(rep(2, dsize))), 
    ...
    geom_text(mapping = aes(x = sufficient50_2, y = 5e-7, label = "50% 2-grams"), angle = 90, size = 4, vjust = -0.4) +
    geom_text(mapping = aes(x = sufficient90_2, y = 5e-7, label = "90% 2-grams"), angle = 90, size = 4, vjust = -0.4)    
```
...
# ... for 2-grams
freqs <- loadFrequencies("../results/dfm/generalDfm2.dat", "../results/dfm/freq2.dat")
g <- g + geom_boxplot(data = data.frame(x = rep(2, length(freqs)), y = freqs / sum(freqs)),  mapping = aes(x = x, y = y, group = x)) + scale_y_continuous(trans = "log")
