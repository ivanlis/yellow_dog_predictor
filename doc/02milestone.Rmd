...
g <- ggplot() + geom_boxplot(data = data.frame(x = rep(1, length(freqs)), y = log(freqs)),  mapping = aes(x = x, y = y, group = x))
...
g <- g + geom_boxplot(data = data.frame(x = rep(2, length(freqs)), y = log(freqs)),  mapping = aes(x = x, y = y, group = x))
